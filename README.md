...
| `JWT_PRIVATE_KEY_FILE` | Path to PKCS#8 private key (Ed25519, ECDSA P-256, or RSA) |
| `JWT_SECONDARY_KEY_FILE` | Secondary key for zero-downtime rotation (may use a different algorithm) |
| `JWT_SIGNING_ALG` | `EdDSA` (default), `ES256`, or `RS256` — signing algorithm for ephemeral keys and new deployments |
| `TLS_ENABLED` / `TLS_CERT_FILE` / `TLS_KEY_FILE` | Serve HTTPS directly from Go (no nginx) with hybrid post-quantum X25519MLKEM768 key exchange |
...
- **Access Token**: Short-lived JSON Web Token (1 minute TTL) signed with the configured algorithm (`JWT_SIGNING_ALG`: EdDSA by default, ES256 or RS256 optional) set as an `httpOnly`, `Secure` (in prod), same-site cookie. It is used to authorize stateless resource requests.
