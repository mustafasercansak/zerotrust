...
## Runtime Session Policy (System Settings)

Session behavior is configured via admin settings stored in `system_settings`.

| Setting Key | Default | Description |
|---|---:|---|
| `session_idle_timeout_seconds` | `300` | Idle window for regular users. Refresh is allowed only within this window since last activity. |
| `session_idle_timeout_seconds_admin` | `180` | Stricter idle window for admin users. |
| `session_absolute_timeout_seconds` | `28800` | Absolute session lifetime (8 hours). Refresh does not extend this deadline. |

Related behavior:

- Access token TTL is 1 minute.
- If idle timeout is exceeded, refresh fails and the user must re-authenticate.
- If absolute timeout is reached, refresh fails even if the user is active.
